...
function CartToast({
  ...
  return createPortal(
    <div
      className={`fixed top-[70px] right-4 z-[9999] transition-all duration-400 ${
        exiting
    ...
    >
      <div className="bg-white rounded-xl shadow-[0_8px_40px_rgba(0,0,0,0.15)] border border-neutral-100 overflow-hidden w-[300px] sm:w-[340px]">
        {/* Top accent bar */}
        <div className="h-[3px] bg-gradient-to-r from-[#C28D10] via-[#E2B84B] to-[#C28D10]" />

        <div className="p-4 flex items-center gap-3.5">
          {/* Checkmark circle */}
          <div
            className="flex-shrink-0 w-9 h-9 rounded-full bg-[#13382C] flex items-center justify-center"
            style={{
          ...
          {/* Product thumbnail */}
          <div className="flex-shrink-0 w-[50px] h-[60px] bg-[#f8f8f8] rounded-lg overflow-hidden relative border border-neutral-100">
            <Image
              src={productImage}
              alt={productName}
              fill
        ...
        <div className="h-[2.5px] bg-neutral-100 relative overflow-hidden">
          <div
            className="absolute inset-y-0 left-0 bg-gradient-to-r from-[#C28D10] to-[#E2B84B]"
            style={{
    ...
    <>
      ...
      {(variant === "both" || variant === "desktop") && (
        <div className="absolute inset-x-0 bottom-0 p-3 opacity-0 transform translate-y-2 transition-all duration-300 ease-in-out group-hover/card:opacity-100 group-hover/card:translate-y-0 hidden lg:flex justify-center z-10">
          <button
            onClick={handleClick}
            className={`flex items-center justify-center gap-2 uppercase text-[11px] font-bold tracking-[0.12em] py-[10px] px-4 w-full transition-all duration-300 rounded-[4px] ${
              added
      ...
      {(variant === "both" || variant === "mobile") && (
        <button
          onClick={handleClick}
          className={`lg:hidden mt-4 flex items-center justify-center gap-2 border uppercase text-[11px] font-bold tracking-[0.12em] py-[10px] px-4 transition-all duration-300 w-full rounded-[4px] ${
            added
